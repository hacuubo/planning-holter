...
import {
  ...
  paques, reculerJoursOuvres, decaler, horodatageEnMinutes, heure, minutes,
...
test('horaires d’ouverture par jour', () => {
  // Mardi : 07:45 -> 18:00
  const mardi = creneauxDuJour('2026-08-25');
  assert.equal(mardi[0], '07:45');
  assert.equal(mardi.at(-1), '18:00');
  assert.equal(mardi.length, 42);

  // Lundi : reprise à 08:00
  const lundi = creneauxDuJour('2026-08-24');
  assert.equal(lundi[0], '08:00');
  assert.equal(lundi.at(-1), '18:00');

  // Samedi : dernier geste à 11:45
  const samedi = creneauxDuJour('2026-08-22');
  assert.equal(samedi[0], '07:45');
  assert.equal(samedi.at(-1), '11:45');
  assert.equal(samedi.length, 17);
